...
class Kpi {
	...
	public static function getSection(\Model\QmailKpi $oKpi, $bForMail = false){
		...
		$sError = '';
		try{
			$aQueryData = self::_getData($oKpi->QUERY, $oKpi->GROUP, $oKpi->CONNEXION, $aParam);
		}catch(\Exception $e){
		...
		try{
			$aQueryDataCompare = self::_getData($oKpi->QUERY_COMPARE, $oKpi->GROUP, $oKpi->CONNEXION, $aParam);
		}catch(\Exception $e){
			$sError .= '<table width="100%" border="0" cellpadding="0" cellspacing="0" style="padding-bottom: 10px;"><tr><td style="padding: 7px; font-family: Arial, Helvetica, sans-serif; font-size: 16px; color: #ffffff; font-weight: bold; background-color: #dd0000;">'.$e->getMessage().'</td></tr></table>';
		}
		...
		if(!empty($aQueryData)){
			...
			if($oKpi->isModeHtml()){
				$sContent .= self::_getHtml($oKpi, $aQueryData, $aQueryDataCompare);
			}elseif($oKpi->isModeGraph()){
				$sContent .= self::_getGraph($oKpi, $aQueryData, $bForMail);
			...
			if($bForMail && $oKpi->ATTACH_CSV == 1){
				self::_attachCsvFile($oKpi, $aQueryData);
			}
	...
	private static function _attachCsvFile(\Model\QmailKpi $oKpi, $aQueryData){
	...
	private static function _getHtml(\Model\QmailKpi $oKpi, $aQueryData, $aQueryDataCompare){
		$aData = array(
			'oKpi' => $oKpi,
	...
	private static function _getGraph(\Model\QmailKpi $oKpi, $aQueryData, $bForMail = false){
		if($oKpi->isGraphModeBar()){
			$img = self::_graphBarPlot($oKpi, $aQueryData);
		}elseif($oKpi->isGraphModePie()){
			$img = self::_graphPie($oKpi, $aQueryData);
		}elseif($oKpi->isGraphModeLine()){
			$img = self::_graphLine($oKpi, $aQueryData);
		}
		if($bForMail){
	...
	private static function _graphPie(\Model\QmailKpi $oKpi, $aQueryData)
	{
	...
	private static function _graphBarPlot(\Model\QmailKpi $oKpi, $aQueryData)
	{
		require_once (APPPATH.'/vendor/jpgraph/src/jpgraph.php');
	...
	private static function _graphLine(\Model\QmailKpi $oKpi, $aQueryData)
	{
		require_once (APPPATH.'/vendor/jpgraph/src/jpgraph.php');
	...
	private static function _getData($sQuery, $sGroup, $sConnexion, $aParam){
		if(!empty($sQuery)){
			// query
			if(is_array($aParam)){
