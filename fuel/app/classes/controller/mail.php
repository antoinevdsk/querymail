...
class Controller_Mail extends \Controller
{
	 ...
	 * @var array
	 */
	protected $_aAttachedFiles = array();

	public function action_index($iMailId = null)
	{
		$oMail = $this->_getMail($iMailId);
	...
	{
		$sMail = $this->_generateMail($iMailId, false);
		if(empty($sMail)){
	...
	public function action_send($iMailId)
	{
		...
		$sMail = $this->_generateMail($iMailId, true);
		if(!empty($sMail)){
			$oMailKpi = $this->_getMail($iMailId);
	...
	private function _generateMail($iMailId, $bForMail)
	{
	...
	private function _attachFiles($oMail) {
		foreach($this->_aAttachedFiles as $aFile) {
			// Rename file
	...
	private function _getMail($iMailId = null)
	{
		$oMail = false;
		if($iMailId){
