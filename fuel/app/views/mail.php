...
<section id="mainWrapper">
	<form class="form-horizontal" role="form" method="POST" action="/mail/save/<?=$oMail->MAIL_ID;?>">
	  ...
	  <div class="form-group">
		...
		<div class="col-sm-10">
		  <input name="TO" class="form-control" placeHolder="Email addresses (separated by comma)" value="<?=$oMail->TO;?>">
		</div>
	  </div>
	  ...
	  <div class="col-sm-offset-1 col-sm-10" style="padding-left:0; padding-right:0;">
		<div class="panel panel-default">
			...
			<div class="panel-body">
			  <?php if($oMail->MAIL_ID > 0):?>
					<p><a href="/kpi/index/<?=$oMail->MAIL_ID;?>" class="btn btn-success"><span class="fa fa-plus"></span> Create a new kpi</a></p>
					...
					<?php foreach ($aKpiDatas as $oMailKpi):?>
						<tr>
							...
							<td>
								<a href="/kpi/index/<?=$oMail->MAIL_ID;?>/<?=$oMailKpi->KPI_ID;?>" class="btn btn-xs btn-primary" title="Edit"><span class="fa fa-edit"></span></a>
								<a href="/kpi/delete/<?=$oMail->MAIL_ID;?>/<?=$oMailKpi->KPI_ID;?>" class="btn btn-xs btn-danger confirm" title="Delete"><span class="fa fa-trash"></span></a>
							</td>
						</tr>
					<?php endforeach;?>
					</table>
		...
		<div class="form-group">
			...
			<button type="submit" class="btn btn-primary"><span class="fa fa-save"></span> <?php if($oMail->MAIL_ID > 0):?>Update<?php else:?>Save<?php endif;?></button>
			<?php if($oMail->MAIL_ID > 0):?>
			<a href="/mail/preview/<?=$oMail->MAIL_ID;?>" target="_blank" class="btn btn-warning"><span class="fa fa-search"></span> Preview</a>
			<a href="/mail/send/<?=$oMail->MAIL_ID;?>" class="btn btn-primary sendMail"><span class="fa fa-send"></span> Send now</a>
			<a href="/mail/delete/<?=$oMail->MAIL_ID;?>" class="btn btn-danger confirm"><span class="fa fa-trash"></span> Delete</a>
			<?php endif;?>
			</div>
		</div>
	</form>
