...
<section id="mainWrapper">
    <form class="form-horizontal" role="form" method="POST" id="kpiForm"
          action="/kpi/save/<?= $oKpi->MAIL_ID; ?>/<?= $oKpi->KPI_ID; ?>">
        ...
        <div class="row form-group">
            <div class="col-md-offset-1 col-md-10">
                <div class="alert alert-warning" style="padding:3px 10px; margin-bottom:5px;">For graph, only first
                    column can be a string, the others must be integer or floating values. In Pie mode, only first 2
                    columns are used.
                </div>
            </div>
            <div class="col-md-offset-1 col-md-5">
                <label for="query" class="control-label">Query</label>
                <div class="form-control console query" id="query"
                     style="height:220px;width:100%"><?= htmlentities($oKpi->QUERY); ?></div>
            </div>
            <div class="col-md-5">
                <label for="query_compare" class="control-label">Comparing query</label>
                <div class="form-control console query" id="query_compare"
                     style="height:220px;width:100%"><?= htmlentities($oKpi->QUERY_COMPARE); ?></div>
            </div>
            <div class="col-md-offset-1 col-md-10">
                <div class="alert"
                     style="background-color:#F5F5F5;border:1px solid #ccc; padding:3px 10px; margin-top:5px;margin-bottom:0px;">
            ...
            <div class="col-md-offset-1 col-md-10">
                <input name="JSON_TEST_PARAM" class="form-control" placeHolder="Test parameters (json format)"
                       style="margin-top:5px;" value="<?= $oKpi->JSON_TEST_PARAM; ?>">
            </div>
        </div>
        ...
        <div class="row form-group">
            ...
            <div class="col-md-4">
                <input name="ATTACH_CSV" id="ATTACH_CSV" type="checkbox"
                       value="1" <? if ($oKpi->ATTACH_CSV == 1) echo 'checked="checked"'; ?>">
            </div>

            <label for="DISPLAY_QUERIES" class="col-md-2 control-label">Display queries</label>
            <div class="col-md-1">
                <input name="DISPLAY_QUERIES" id="DISPLAY_QUERIES" type="checkbox"
                       value="1" <? if ($oKpi->DISPLAY_QUERIES == 1) echo 'checked="checked"'; ?>">
            </div>

            <label for="DIFF_PERCENT" class="col-md-1 control-label">Diff in percent</label>
            <div class="col-md-2">
                <input name="DIFF_PERCENT" id="DIFF_PERCENT" type="checkbox"
                       value="1" <? if ($oKpi->DIFF_PERCENT == 1) echo 'checked="checked"'; ?>">
            </div>
        </div>
        ...
        <div class="row form-group">
            ...
            <div class="col-md-4">
                <input name="IS_ACTIVE" id="IS_ACTIVE" type="checkbox"
                       value="1" <? if ($oKpi->IS_ACTIVE == 1) echo 'checked="checked"'; ?>">
            </div>

            <label for="INVERT_COLOR" class="col-md-2 control-label">Invert color</label>
            <div class="col-md-1">
                <input name="INVERT_COLOR" id="INVERT_COLOR" type="checkbox"
                       value="1" <? if ($oKpi->INVERT_COLOR == 1) echo 'checked="checked"'; ?>">
            </div>

            <label for="FORMAT_INTEGER" class="col-md-1 control-label">Format integers</label>
            <div class="col-md-2">
                <input name="FORMAT_INTEGER" id="FORMAT_INTEGER" type="checkbox"
                       value="1" <? if ($oKpi->FORMAT_INTEGER == 1) echo 'checked="checked"'; ?>">
            </div>
        </div>
        ...
        <div class="row form-group">
            <div class="col-md-offset-1 col-md-10">
                <button type="submit" class="btn btn-primary"><span
                        class="fa fa-save"></span> <?php if ($oKpi->KPI_ID > 0): ?>Update<?php else: ?>Save<?php endif; ?>
                </button>
                <a href="#preview" class="btn btn-warning kpiPreview"><span class="fa fa-search"></span> Preview</a>
                <a href="/mail/index/<?= $oKpi->MAIL_ID; ?>" class="btn btn-primary"><span class="fa fa-close"></span>
                    Close</a>
            </div>
        </div>
    </form>
